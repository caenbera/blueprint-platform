...
export function CollapsibleSidebar({
  ...
  const [collapsed, setCollapsed] = useLocalStorageState("blueprint:sidebar-collapsed", false);
  ...
  return (
    <div
      className={cn(
        "bg-sidebar flex h-full shrink-0 flex-col border-r transition-[width] duration-200",
        collapsed ? "w-16" : "w-64",
      )}
    >
      ...
      <button
        ...
        className={cn(
          "text-muted-foreground hover:bg-muted hover:text-foreground mx-3 mb-2 flex items-center gap-2 rounded-md px-2 py-1.5",
      ...
      >
        ...
        {!collapsed && <span className="text-caption font-medium">Colapsar</span>}
      </button>
      ...
      <nav className="flex-1 space-y-4 overflow-y-auto px-3 pb-4">
        {groups.map((group) => {
          ...
          const groupCollapsed =
            !ungrouped && !collapsed && (collapsedGroups[group.label] ?? false);
          const items = (
          ...
          if (ungrouped || collapsed) {
            return <div key={ungrouped ? "ungrouped" : group.label}>{items}</div>;
          }
      ...
      <div className={cn("border-t px-3 py-3", collapsed && "px-2")}>
        {footerTop && !collapsed && <div className="mb-3">{footerTop}</div>}
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              ...
              className={cn(
                "hover:bg-muted flex w-full items-center gap-2.5 rounded-md px-1 py-1 text-left",
